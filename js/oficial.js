...
function obtenerSeleccionados(nombreCampo){return Array.from(document.querySelectorAll(`input[name="${nombreCampo}"]:checked`)).map(i=>i.value);}
function marcarSeleccionados(nombreCampo, valores){if(!Array.isArray(valores))return;document.querySelectorAll(`input[name="${nombreCampo}"]`).forEach(i=>i.checked=valores.includes(i.value));}
function mostrarAviso(mensaje,tipo){const aviso=document.getElementById("aviso");aviso.style.display="block";aviso.className="aviso "+tipo;aviso.innerText=mensaje;}
function generarIdOficial(uid){return "OF-"+uid.slice(0,6).toUpperCase();}

function cargarFormulario(data){
    const campos = ["idOficial","nombre","apellidos","dni","fechaNacimiento","email","telefono","provincia","municipio","codigoPostal","federacion","numeroLicencia","vehiculoPropio","distanciaMaxima","observaciones"];
    campos.forEach(id=>{const el=document.getElementById(id); if(el) el.value=data[id] || "";});
...
async function cargarPerfil(){
    const snap=await getDoc(doc(db,"oficiales",usuarioActual.uid));
    if(snap.exists()){cargarFormulario(snap.data());}
    else{
...
async function guardarPerfil(){
    ...
    if(!nombre||!apellidos||!provincia||!municipio){mostrarAviso("Rellena nombre, apellidos, provincia y municipio.","error");return;}
    const data={
    ...
    try{await setDoc(doc(db,"oficiales",usuarioActual.uid),data,{merge:true});mostrarAviso("Perfil guardado correctamente.","ok");}
...
onAuthStateChanged(auth,async(user)=>{if(!user){window.location.href="login.html";return;}usuarioActual=user;document.getElementById("usuarioInfo").innerText="Sesión iniciada: "+user.email;await cargarPerfil();});
